...
for influencer in influencers[index_last_influencer:]:
    ...
    while next_cursor != 0:
        try:
            ...
            follower_list = api_handle.followers_ids(
                    influencer, 
                    cursor = next_cursor, 
                    count = 5000, 
                    stringify_ids = True
                )
            next_cursor = follower_list[1][1]
            next_token_logger.info("{}, {}".format(next_cursor, influencer))
            followers_ids_loggers[influencer].info("\n".join(follower_list[0]))
            
            general_logger.info("Got {} followers of {}".format(len(follower_list[0]), influencer))
            ...
            if user_lookups_remaining <= int(len(follower_list[0])/100) + 1:
            ...
            general_logger.info("Now getting user information for {} users".format(len(follower_list[0])))
            for user_id_group in chunk_ids(follower_list[0]):
